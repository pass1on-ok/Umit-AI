    ...
    <div className="w-full max-w-6xl mx-auto flex flex-col gap-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage platform users, content, and application global settings.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="shadow-custom border-border bg-card">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Uptime SLA Maintained</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="shadow-custom border-border bg-card h-[500px] flex flex-col overflow-hidden">
        <CardHeader className="border-b border-border/50 bg-muted/30">
          <div className="flex justify-between items-center">
            <CardTitle className="flex items-center gap-2"><Users className="w-5 h-5"/> User Management Directory</CardTitle>
                ...
                <tr key={idx} className="hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-xs border ${idx === 2 ? 'bg-primary/10 border-primary/20 text-primary' : idx === 0 ? 'bg-chart-2/10 border-chart-2/20 text-chart-2' : 'bg-muted border-border text-muted-foreground'}`}>
                      {idx === 2 ? 'SYS_ADMIN' : idx === 0 ? 'PSYCHOLOGIST' : 'PATIENT'}
                    </span>
                  </td>
                  <td className="px-6 py-4"><span className="text-green-500 text-xs font-bold">• Active</span></td>
