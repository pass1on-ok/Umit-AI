...
import { submitTest, type TestAnswerPayload } from '@/services/api';
...
const TestInterface = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload: TestAnswerPayload = { answers: currentAnswers };
      await submitTest(payload);
      navigate('/test/results');
    } catch (err) {
      setError((err as Error).message || 'Unable to submit test.');
    } finally {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto flex flex-col gap-6 px-4">
      ...
      {isSubmitting ? <LoadingSpinner text="Submitting your answers..." variant="page" /> : null}
      ...
      {!isSubmitting ? (
        <>
          ...
          <div className="flex justify-end mt-4">
            <Button size="lg" onClick={handleSubmit} disabled={Object.keys(currentAnswers).length < questions.length} className="gap-2 px-8">
